...
[cmdop-claude](https://pypi.org/project/cmdop-claude/) — markolofsen의 접근법. 그래프로는 Merkle 트리를 사용합니다. 핵심 아이디어: 거의 무료인 중국 LLM을 백그라운드에서 돌려 `.claude` 폴더를 정리 — 메인 모델을 위한 컨텍스트를 준비하는 것입니다.
...
macOS용 기성 솔루션이 있습니다 — [Open Screenshot](https://openscreenshot.suenot.com/). 이 유틸리티는 해상도가 축소된 형식으로 바로 스크린샷을 찍어, 수동으로 리사이즈할 필요가 없습니다.
