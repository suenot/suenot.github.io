...
description: "分步教程：使用 NotebookLM 生成音频和幻灯片，然后自动组装成带字幕、章节时间戳和元数据的 YouTube 视频——无需视频编辑器，完全不用剪辑。"
pubDate: 2026-05-09
...
- 🎬 [Walk-Forward Optimization: The Only Honest Strategy Test](https://www.youtube.com/watch?v=y_cC6LWXFKM) — **video-youtube-prepare** 示例：NotebookLM 生成了成品视频，脚本为其准备了元数据（标题、描述、标签、时间戳、字幕）
...
**工具：** [video-youtube-prepare](https://github.com/suenot/video-youtube-prepare)
...
- 🔧 [video-youtube-prepare](https://github.com/suenot/video-youtube-prepare) — 生成 YouTube 元数据
...
> **重要说明：** `video-maker` 和 `video-youtube-prepare` 不是开箱即用的成品，而是**模板和起点**。它们在基本场景下可以直接使用，但设计目的是让你用任何 AI 代理（Claude Code、Codex、Cursor 等）根据自己的需求进行定制。Fork 它们，适配你的品牌和内容风格。
