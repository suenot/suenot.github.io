...
[cmdop-claude](https://pypi.org/project/cmdop-claude/) 是 markolofsen 的方案。图谱方面使用了 Merkle 树。核心思路：在后台运行几乎免费的国产 LLM 来整理 `.claude` 文件夹——为主模型准备好上下文。
...
macOS 上有现成方案——[Open Screenshot](https://openscreenshot.suenot.com/)。这个工具直接以低分辨率格式截图，不需要手动调整大小。
