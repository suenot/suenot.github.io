...
Видео: [Your AI Doesn't Know What Happened Last Month. One Command Fixes It](https://www.youtube.com/watch?v=4HpQh1heIkw)
...
Я сверил архитектурные утверждения исходного видео с репозиторием. Несколько не совпали с кодом. В расхождениях видны инженерные компромиссы, которые не попали в аккуратное объяснение из видео.
...
Исходное видео утверждало, что разработчики "полностью отказались от долларовых объемов и перешли на чистые вероятности, потому что объем искажается китами". Код показывает обратное. Объем имеет наибольший вес в формуле, 0.50, а логарифмическое сжатие ограничивает влияние крупных участников.

## Дедупликация использует коэффициент Жаккара, а не эмбеддинги

В видео также говорилось, что в третьей версии появились "векторные эмбеддинги для кроссплатформенной семантической кластеризации", которые объединяют обсуждение на Reddit, спор в X и комментарии на YouTube в одну сущность.
...
В исходном видео два бага были представлены как решенные, хотя оба тикета остаются открытыми на GitHub. Третий тикет уже закрыт и показывает ошибку другого типа.
...
Исходное видео само было синтезировано ИИ. Оно уверенно назвало два открытых тикета закрытыми, заявило об отказе от ранжирования по объему и описало дедупликатор Жаккара как систему векторных эмбеддингов. Проверка этих утверждений через GitHub API и исходный код заняла около девяноста секунд. Поэтому описание реализации в этой статье отличается от видео.
