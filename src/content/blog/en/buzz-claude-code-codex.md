...
description: "Your Claude Code is in one terminal, your Codex is in another, and you are the copy-paste layer between them. Buzz is Block's open-source workspace where agents join channels as members with their own keys — here is how the ACP harness wires them up, what it actually buys you, and what is still alpha."
pubDate: 2026-08-08
...
[Buzz](https://github.com/block/buzz) deletes that job. It's a self-hostable workspace from Block — Apache-2.0, around 25,000 GitHub stars — where humans and agents sit in the same channels. Not a bot that posts into your chat: a member, with its own identity, its own channel memberships, and its own audit trail.
...
Anything that speaks ACP over stdio plugs in. Three adapters are documented today:
...
One note from the docs worth having in advance: `codex-acp` always tries a ChatGPT WebSocket login first and logs a `426 Upgrade Required`. It's expected and non-fatal — it falls back to `OPENAI_API_KEY`, which is why the docs insist on an API key rather than a ChatGPT subscription.

The protocol is the whole point. You aren't buying into a vendor's agent; you're pointing a generic harness at whatever agent you already pay for. The subscription you have is the one that powers this. There's no new model bill.
...
Every agent gets its own Nostr keypair. That keypair *is* its identity — generated with `buzz-admin generate-key`, then registered on the relay as a member (a kind:13534 membership event). Running three agents means minting three keypairs. The harness then discovers only the channels that agent is actually a member of.
...
**A release that writes itself.** A YAML workflow fires on a tag, an agent reads the merged PRs from the project channels, drafts the release notes, posts them for review, gets a 👍 reaction, and ships. Workflows trigger on messages, reactions, schedules or webhooks — a reaction as an approval gate is a nice trick, because it's a control humans already use without being taught.
...
*Source: this article was written from a walkthrough on the [HinkoK](https://www.youtube.com/watch?v=ZA8XamfjuYo) channel, with all technical claims verified against the [block/buzz](https://github.com/block/buzz) repository and its `buzz-acp` documentation.*
